...
/**
 * Confirmation dialog for copying a question between kho cá nhân ↔ kho trường.
 * The destination is the opposite of the source kho:
 *   - source = campus  → personal (auto-approved, available ngay cho mình)
 *   - source = personal → campus  (status = pending, cần TBM / Admin campus duyệt)
 */
export function CopyQuestionDialog({ question, onClose, onConfirm }: Props) {
  // Render nothing while there is no question. Mounting a Dialog with closed
  // state but no DialogTitle triggers a Radix a11y warning, and there's no
  // useful transition state to preserve here either.
  if (!question) return null;

  const sourceIsCampus = question.kho === "campus";
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Tạo bản sao của câu hỏi <span className="font-mono">{question.id}</span>{" "}
            sang kho ngược lại.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="my-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
          <div className="rounded-lg border bg-card px-3 py-2.5 text-center">
            <SourceIcon className="mx-auto h-4 w-4 text-foreground/65" strokeWidth={1.85} />
          ...
          <ArrowRight className="h-5 w-5 text-muted-foreground" strokeWidth={1.85} />
          <div
            className={
              targetTone === "emerald"
        ...
        {sourceIsCampus ? (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50/60 px-3 py-2 text-[12px] leading-relaxed text-emerald-800">
            <span className="font-semibold">Tự duyệt</span> · Bản sao sẽ vào kho cá
            nhân của bạn với trạng thái <span className="font-semibold">Đã duyệt</span>,
            dùng được ngay.
          </div>
        ) : (
          <div className="rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2 text-[12px] leading-relaxed text-amber-800">
            <span className="font-semibold">Cần duyệt</span> · Bản sao sẽ vào kho
            trường với trạng thái <span className="font-semibold">Chờ duyệt</span>;
            TBM hoặc Admin campus xét duyệt như luồng thông thường.
          </div>
        )}
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onConfirm(question)}>
