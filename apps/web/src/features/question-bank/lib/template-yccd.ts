/**
 * Tài liệu Word MẪU cho kiểu soạn đề theo mã YCCĐ — phần dựng nội dung.
 *
 * Tách khỏi route `/api/import/yccd-template` để ca hồi quy đọc ngược được
 * chính file mà giáo viên tải về (route kéo theo `next/server`, không nạp
 * được trong Node thuần). Đúng khuôn `template-co-ban.ts` đã có.
 *
...
export function buildYccdTemplate(): Document {
  return new Document({
    sections: [
      {
        children: [
          ...
          p([
            t("   G", { bold: true }),
            t("   Câu nhóm — một đề bài / ngữ liệu chung, nhiều câu hỏi phụ bên dưới"),
          ]),
          blank(),
          ...
          // ── Ví dụ ────────────────────────────────────────────────────
          h("VÍ DỤ ĐỦ NĂM LOẠI CÂU (xoá và thay bằng đề của bạn)", HeadingLevel.HEADING_1),
          blank(),
          ...
          p([t("A", { underline: true }), t(". 70%.")]),
          plain("B. 30%."),
          plain("C. 90%."),
          plain("D. 50%."),
          plain("<2> Những vai trò nào sau đây là của nước? (chọn nhiều đáp án)"),
          p([t("A", { underline: true }), t(". Dung môi hòa tan nhiều chất.")]),
          p([t("B", { underline: true }), t(". Tham gia điều hòa nhiệt độ cơ thể.")]),
          ...
          plain("<Key=70>"),
          note(
            "Chữ G cho biết đây là câu nhóm. Ngữ liệu chung viết ngay sau mã; mỗi câu hỏi phụ mở bằng <1>, <2>… Dạng của từng ý tự nhận ra: có <Key=…> là trả lời ngắn, còn lại đếm phương án gạch chân (1 gạch = một đáp án, từ 2 gạch = nhiều đáp án).",
          ),
          blank(),
          blank(),
